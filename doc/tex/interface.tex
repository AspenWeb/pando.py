...
are meaningful to your application.
...
is requested.}{\code{index.html, index.htm}}
%
...
any time module source files change on the filesystem.}{\code{development}}
...
\section{The \class{config} object}
\label{config}

Aspen parses and harmonizes all command-line, configuration file, and
environment settings before it loads your plugins. Aspen provides an API for
this configuration via an \class{aspen.configuration.Configuration} instance,
which your plugin modules may access with:
...
This object exposes the various underlying configuration parsers as these
members:
...
An instance of \code{aspen.configuration.ConfFile}, which wraps the standard
library's \code{ConfigParser.RawConfigParser} class. The object supports both
attribute and key access to return dictionaries corresponding to each section of
the \file{aspen.conf} file. If a section doesn't exist, an empty dictionary is
returned.

Your application is free to use the \file{aspen.conf} file for it's own
configuration, via this object. As a minimal example, this \file{aspen.conf}:
...
An instance of \code{aspen.configuration.Paths}, which is a container for
various paths, all normalized and absolute: \code{root} (the website's
filesystem root), \code{__} (its magic directory); \code{lib} (the site's local
Python library); \code{plat} (the local platform-specific Python library).
\end{memberdesc}


\subsection{Settings}

Furthermore, \code{aspen.config} exposes specific configuration settings as
these members:

\begin{memberdesc}[]{address}
A tuple (for \code{AF_INET} and \code{AF_INET6} address) or string (for
\code{AF_UNIX}) given the address to which Aspen is bound.
\end{memberdesc}
...
{\code{RawConfigParser}}{In addition to the API above, \code{aspen.config.conf}
also exposes the \code{RawConfigParser} API.}

\seelink{http://docs.python.org/lib/module-optparse.html} {\code{optparse}}{The
documentation for optparse is rather, um, convoluted. Good luck!}
