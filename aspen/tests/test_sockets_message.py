...
def test_from_bytes_too_few_colons_raises_Response():
    response = assert_raises(Response, Message.from_bytes, '3::')
    expected = "Malformed message: 3::"
...
def test_from_bytes_non_digit_type_raises_Response():
    response = assert_raises(Response, Message.from_bytes, 'foo:::')
    expected = "Message type not in 0..8: foo"
    actual = response.body
    assert actual == expected, actual

def test_from_bytes_type_too_small_raises_Response():
    response = assert_raises(Response, Message.from_bytes, '-1:::')
    expected = "Message type not in 0..8: -1"
    actual = response.body
    assert actual == expected, actual

def test_from_bytes_type_too_big_raises_Response():
    response = assert_raises(Response, Message.from_bytes, '9:::')
    expected = "Message type not in 0..8: 9"
    actual = response.body
    assert actual == expected, actual
...
def test_event_data_without_name_raises_Response():
    response = assert_raises( Response
                            , Message.from_bytes
                            , '5:::{"noom": "bar", "args": []}'
                             )
    expected = "Event message must have 'name' key."
    actual = response.body
    assert actual == expected, actual

def test_event_data_without_args_raises_Response():
    response = assert_raises( Response
                            , Message.from_bytes
                            , '5:::{"name": "bar", "arrrrgs": []}'
                             )
    expected = "Event message must have 'args' key."
    actual = response.body
    assert actual == expected, actual

def test_event_data_with_reserved_name_raises_Response():
    response = assert_raises( Response
                            , Message.from_bytes
                            , '5:::{"name": "connect", "args": []}'
                             )
    expected = "Event name is reserved: connect."
    actual = response.body
    assert actual == expected, actual
