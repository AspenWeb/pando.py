...
class Configurable(object):
    ...
    def configure(self, **kwargs):
        ...
        # mimetypes
        pandos_mimetypes = os.path.join(os.path.dirname(__file__), 'mime.types')
        mimetypes.knownfiles += [pandos_mimetypes]
        ...
        # log appropriately
        pando.log_dammit(os.linesep.join(msgs))
        ...
        if self.project_root is None:
            pando.log_dammit("project_root not configured (no template bases, "
                             "etc.).")
        else:
            # canonicalize it
            if not os.path.isabs(self.project_root):
                pando.log_dammit("project_root is relative to CWD: '%s'."
                                 % self.project_root)
            ...
            pando.log_dammit("project_root set to %s." % self.project_root)
        ...
        # load bodyparsers
        self.body_parsers = {
            "application/x-www-form-urlencoded": pando.body_parsers.formdata,
            "multipart/form-data": pando.body_parsers.formdata,
            self.media_type_json: pando.body_parsers.jsondata
            }
    ...
    def show_renderers(self):
        pando.log_dammit("Renderers (*ed are unavailable, CAPS is default):")
        ...
        for name, factory in self.renderer_factories.items():
            ...
            pando.log_dammit(" %s%s%s" % (star, name, error))
        ...
        if isinstance(default_renderer, ImportError):
            ...
            pando.log_dammit(msg % self.renderer_default)
